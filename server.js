...
server.listen(PORT, async () => {
  ...
  // Brain picks its own tickers from market scan
  ...
  // Safety net — if AI scan returned nothing (API error on boot), seed a few starters
  // Brain will replace these immediately on next rotate cycle
  if (tickers.length === 0) {
    console.log('⚠️ Scan returned no tickers — seeding starters while brain warms up...');
    const starters = [
      ...
      { sym:'MARA',  price:15.20, float:320,  short:25, sector:'Crypto',  catalyst:'Bitcoin correlated', ai_verdict:'WATCH', confidence:60 },
      { sym:'MVST',  price:1.85,  float:92,   short:20, sector:'Battery', catalyst:'EV battery play', ai_verdict:'WATCH', confidence:60 },
      { sym:'CYTO',  price:3.20,  float:45,   short:15, sector:'Biotech', catalyst:'Biotech momentum', ai_verdict:'WATCH', confidence:60 },
    ];
    for (const s of starters) addTicker(s);
    console.log('✅ Starters seeded — brain will rotate to better picks shortly');
  }
