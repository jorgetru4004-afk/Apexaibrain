...
// ── PRICE SIMULATION (replaced by Alpaca when connected) ──
...
function autoAddTicker(disc) {
  ...
  broadcast('TICKER_ADDED', { sym, stock: STOCKS[sym], bot: bots[sym] });
}
...
async function rotateRoster() {
  ...
  broadcast('ROSTER_UPDATE', { tickers, tickerHealth, rotationLog });
}
...
// Price simulation every 3 seconds (while market open or always in paper mode)
