...
// ── STATE ──
let tickers      = []; // Always start empty — brain fills from market scan, ignores old roster.json
...
let newsItems        = [];
let rotationLog      = [];
let tradeCooldown    = {}; // sym -> timestamp of last trade, prevents spam
...
// No hardcoded tickers — brain picks real ones from market scan on startup
// Any saved tickers from roster.json are restored
tickers.forEach(sym => {
  initStock(sym, parseFloat((Math.random()*8+1).toFixed(3)),
    parseFloat((Math.random()*60+5).toFixed(1)),
    parseFloat((Math.random()*30+5).toFixed(1)),
    Math.floor(Math.random()*5e6+500000), 'Discovery', '#00ffff');
});
...
// Price fetching removed — using simulation for display, Alpaca used only for order execution
// This keeps the Alpaca account connection status clean and accurate
...
function runBotLogic(sym) {
  ...
  // ENTRY — cooldown check (5 min between trades on same ticker)
  const lastT = tradeCooldown[sym] || 0;
  if (Date.now() - lastT < 90 * 1000) { b.status = 'COOLDOWN'; return; }
  ...
  tradeCooldown[sym] = Date.now(); // set cooldown so same ticker can't spam
...
// Price simulation every 3 seconds — Alpaca used only for order execution
...
server.listen(PORT, async () => {
  ...
  await fetchRealNews();
  await sleep(2000);
  // Clear any old hardcoded tickers — brain picks fresh ones
  const hardcoded = ['ONDS','BBAI','AZI','JEM','JYZN'];
  hardcoded.forEach(sym => {
    if (tickers.includes(sym)) {
      tickers = tickers.filter(s => s !== sym);
      delete STOCKS[sym]; delete bots[sym]; delete hist[sym];
      delete vols[sym]; delete sentimentMap[sym]; delete aiDecisions[sym];
      console.log('🗑 Cleared hardcoded ticker: ' + sym);
    }
  });
  // Full market scan — brain hunts real opportunities from scratch
  console.log('🔍 Running startup market scan — brain picking real tickers...');
